...
import type { DiffChangeKind, DiffDocumentFormat, DiffOptions, DiffResult, ResolvedFormat, WorkspaceRecord, WorkspaceSnapshot } from "../types";
...
import { compareDocuments, DEFAULT_DIFF_OPTIONS, detectDiffFormat } from "../core/diff";
...
function extensionFor(format: ResolvedFormat) { return format === "text" ? "txt" : format; }
function withResolvedExtension(name: string, format: ResolvedFormat) { return name.replace(/\.[^.]+$/, `.${extensionFor(format)}`); }
function isGeneratedSideName(name: string, side: "before" | "after") { return new RegExp(`^${side}\\.(?:json|xml|sql|txt)$`, "i").test(name); }
...
export function DiffPage({ snapshot, workspaceId, active = false, onTitleChange, onWorkspaceSaved }: { snapshot?: WorkspaceSnapshot; workspaceId?: string; active?: boolean; onTitleChange?: (name: string) => void; onWorkspaceSaved?: (workspace: WorkspaceRecord) => void }) {
  ...
  const [leftNameGenerated, setLeftNameGenerated] = useState(true);
  const [rightNameGenerated, setRightNameGenerated] = useState(true);
  const [format, setFormat] = useState<DiffDocumentFormat>("auto");
  ...
  useEffect(() => {
    if (snapshot?.kind === "diff") {
      ...
      const nextLeftName = snapshot.leftName || t("工作区 - 左侧"); const nextRightName = snapshot.rightName || t("工作区 - 右侧");
      setLeftName(nextLeftName); setRightName(nextRightName);
      setLeftNameGenerated(Boolean(snapshot.leftName && isGeneratedSideName(nextLeftName, "before")));
      setRightNameGenerated(Boolean(snapshot.rightName && isGeneratedSideName(nextRightName, "after")));
    }
  }, [snapshot]);

  const resolvedFormat = format === "auto" ? (detectDiffFormat(leftText) === detectDiffFormat(rightText) ? detectDiffFormat(leftText) : "text") : format;
  const displayedLeftName = leftNameGenerated ? withResolvedExtension(leftName, resolvedFormat) : leftName;
  const displayedRightName = rightNameGenerated ? withResolvedExtension(rightName, resolvedFormat) : rightName;
  ...
  const highlights = useMemo(
    () => resolvedFormat === "sql" && result.normalizedLeft !== undefined && result.normalizedRight !== undefined
      ? { left: [], right: [] }
      : buildDiffHighlights(leftText, rightText, resolvedFormat, filtered),
    [leftText, rightText, resolvedFormat, filtered, result.normalizedLeft, result.normalizedRight],
  );
  ...
  const openSide = async (side: "left" | "right") => {
    ...
    if (side === "left") { setLeftText(file.content); setLeftName(file.name); setLeftNameGenerated(false); } else { setRightText(file.content); setRightName(file.name); setRightNameGenerated(false); }
    onTitleChange?.(`${nextLeftName} vs ${nextRightName}`);
    await addRecentFile({ name: file.name, size: file.size, path: file.path, format: detectDiffFormat(file.content) });
  };
  const exportResult = async (exportFormat: ExportFormat) => {
    try {
      const content = generateDiffExport({ title: t(`${leftName} 与 ${rightName} 对比报告`), leftName, rightName, leftText, rightText, result, options, maskRules, maskEnabled: maskExport && result.format !== "sql" }, exportFormat);
