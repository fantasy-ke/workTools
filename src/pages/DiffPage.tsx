...
import type { DiffChangeKind, DiffOptions, DiffResult, DocumentFormat, WorkspaceSnapshot } from "../types";
...
export function DiffPage({ snapshot, active = false, onTitleChange }: { snapshot?: WorkspaceSnapshot; active?: boolean; onTitleChange?: (name: string) => void }) {
  ...
  const saveCurrentWorkspace = async () => { const name = window.prompt(t("工作区名称"), `${leftName} vs ${rightName}`); if (!name) return; const saved = await saveWorkspace(name, { kind: "diff", leftText, rightText, format, options, leftName, rightName }, maskExport); onTitleChange?.(saved.name); notify(t("工作区已保存")); };
