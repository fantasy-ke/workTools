...
import { Braces, ChevronDown, Clipboard, Download, Eraser, FileCheck2, FileInput, ListTree, Minimize2, Save, ShieldCheck, Sparkles, Undo2 } from "lucide-react";
import type { DocumentFormat, StructureEntry, WorkspaceSnapshot } from "../types";
...
export function FormatPage({ snapshot, active = false, onTitleChange }: { snapshot?: WorkspaceSnapshot; active?: boolean; onTitleChange?: (name: string) => void }) {
  ...
  const [tab, setTab] = useState<"structure" | "problems" | "baseline">("structure");
  ...
  const exportRaw = async () => {
    try {
      const content = maskExport && resolved !== "text" ? maskDocumentText(text, resolved, maskRules) : text;
  ...
  const saveCurrentWorkspace = async () => {
    const name = window.prompt(t("工作区名称"), sourceName.replace(/\.[^.]+$/, ""));
    if (!name) return;
    const saved = await saveWorkspace(name, { kind: "format", text, format, sourceName }, maskExport); onTitleChange?.(saved.name); notify(t("工作区已保存"));
