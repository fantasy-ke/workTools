...
import type { DocumentFormat, ResolvedFormat, StructureEntry, WorkspaceRecord, WorkspaceSnapshot } from "../types";
...
function isGeneratedSourceName(name: string) { return /^(?:未命名|unnamed)\.(?:json|xml|txt)$/i.test(name); }
...
export function FormatPage({ snapshot, workspaceId, active = false, onTitleChange, onWorkspaceSaved }: { snapshot?: WorkspaceSnapshot; workspaceId?: string; active?: boolean; onTitleChange?: (name: string) => void; onWorkspaceSaved?: (workspace: WorkspaceRecord) => void }) {
  ...
  const [sourceNameGenerated, setSourceNameGenerated] = useState(true);
  ...
  useEffect(() => {
    if (snapshot?.kind === "format") {
      const nextSourceName = snapshot.sourceName || t("工作区报文");
      setText(snapshot.text); setFormat(snapshot.format); setSourceName(nextSourceName); setSourceNameGenerated(Boolean(snapshot.sourceName && isGeneratedSourceName(nextSourceName)));
    }
  }, [snapshot]);

  const resolved = resolve(text, format);
  const displayedSourceName = sourceNameGenerated ? sourceName.replace(/\.[^.]+$/, `.${extensionFor(resolved)}`) : sourceName;
  ...
  const openFile = async () => {
    ...
    setText(file.content); setSourceName(file.name); setSourceNameGenerated(false); setFormat("auto"); onTitleChange?.(file.name);
